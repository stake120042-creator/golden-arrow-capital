...
class OTPService {
  ...
  private keyFor(email: string, type: 'signup' | 'login' | 'withdrawal' | 'refund'): string {
    return `${email}|${type}`;
  }
  ...
  public async resendOTP(email: string, type: 'signup' | 'login' | 'withdrawal' | 'refund', firstName?: string): Promise<{ success: boolean; message: string }> {
    try {
  ...
  public async getOTPStatus(email: string, type: 'signup' | 'login' | 'withdrawal' | 'refund'): Promise<{ exists: boolean; expired?: boolean; attempts?: number }> {
    try {
      const normalizedEmail = email.toLowerCase();
  ...
  public async getUserDataFromOTP(email: string, type: 'signup' | 'login' | 'withdrawal' | 'refund'): Promise<any> {
    try {
      const normalizedEmail = email.toLowerCase();
  ...
  public getDebugOTP(email: string, type: 'signup' | 'login' | 'withdrawal' | 'refund'): any {
